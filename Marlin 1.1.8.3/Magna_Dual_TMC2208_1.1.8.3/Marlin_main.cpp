...
inline void gcode_G0_G1(
  ...
  if (IsRunning() && G0_G1_CONDITION) {
	...
	const float echange = destination[E_AXIS] - current_position[E_AXIS];
	 if(echange >0.01){
	  	if(thermalManager.degTargetHotend(0) <160){//jone
			gcode_M25();
			LCD_MESSAGEPGM("TEMP TOO LOW!");
	  		}
	  	}
...
void setup() {
  ...
  //  pinMode(FAN_PIN1, OUTPUT);
	//digitalWrite(FAN_PIN1, 255);
	//analogWrite(FAN_PIN1, 255);
	  
	 
	   //SET_OUTPUT(FAN2_PIN);
		// allows digital or PWM fan output to be used 255
	  //WRITE(FAN2_PIN, 255);
	  //analogWrite(FAN2_PIN, 255);
	//pinMode(FAN_PIN2, OUTPUT);
	//digitalWrite(FAN_PIN2, 255);
	//analogWrite(FAN_PIN2, 255);
