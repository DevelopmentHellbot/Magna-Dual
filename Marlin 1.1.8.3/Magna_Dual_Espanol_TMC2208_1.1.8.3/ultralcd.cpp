...
millis_t next_lcd_update_refresh;
...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_sdcard_stop() {
      ...
      enqueue_and_echo_commands_P(PSTR("G1 F1000 Z20"));
      enqueue_and_echo_commands_P(PSTR("G90"));
      enqueue_and_echo_commands_P(PSTR("G28 XY"));
      enqueue_and_echo_command("G1 F1000 Y210");
...
//int16_t deflect[2];
  ...
  //    if((mixer.rate[NOZZLE0] > 0) && (mixer.rate[NOZZLE0] < 9))
  //    {
  //	  	mixing_factor[NOZZLE0] = RECIPROCAL(8 * 0.01);
 //    mixing_factor[NOZZLE1] = RECIPROCAL(92 * 0.01);
 //     } else if((mixer.rate[NOZZLE1] > 0) && (mixer.rate[NOZZLE1] < 9))
 //     {
//	  	mixing_factor[NOZZLE0] = RECIPROCAL(92 * 0.01);
//	    mixing_factor[NOZZLE1] = RECIPROCAL(8 * 0.01);
 //     } else {
      	mixing_factor[NOZZLE0] = RECIPROCAL(mixer.rate[NOZZLE0] * 0.01);
      	mixing_factor[NOZZLE1] = RECIPROCAL(mixer.rate[NOZZLE1] * 0.01);
   //   }
	  //SERIAL_ECHOLNPAIR("Nozzle0=",mixing_factor[NOZZLE0]);
	  //SERIAL_ECHOLNPAIR("Nozzle1=",mixing_factor[NOZZLE1]);
	  //SERIAL_ECHOLNPAIR("def Nozzle0=",deflect[NOZZLE0]);
	  //SERIAL_ECHOLNPAIR("def Nozzle1=",deflect[NOZZLE1]);
    }
    ...
    inline void _lcd_mixer_toggle_mix() {
	  ...
	  //deflect[NOZZLE0] = mixer.rate[NOZZLE0];
	  //deflect[NOZZLE1] = mixer.rate[NOZZLE1];
    ...
    void lcd_mixer_menu() {
	  ...
	  //mixer.rate[NOZZLE0] = int16_t(deflect[NOZZLE0]);
	  //mixer.rate[NOZZLE1] = 100 - deflect[NOZZLE0];
 ...
 // static void filament_runout_script()
  //	{
//		settings.save();
//		lcd_completion_feedback();
 // 	}
  ...
  void lcd_control_filament_menu() {
    ...
    MENU_ITEM_EDIT_CALLBACK(bool, MSG_RUNOUT_SENSORS, (bool*)&filament_runout_enabled, lcd_store_settings);
	//MENU_ITEM(function, MSG_RESTORE_FAILSAFE, lcd_factory_settings);
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)
    ...
    if (
	  ...
	  const millis_t tms = millis();
	   if (ELAPSED(tms, next_lcd_update_refresh))
		 {
		 next_lcd_update_refresh = tms + LCD_UPDATE_REFRESH_INTERVAL;
		 lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW;
		 } else {
		 lcdDrawUpdate = LCDVIEW_REDRAW_NOW; //old code
		 }
...
void lcd_setforcestatusPGM(const char * const message, int8_t level) {
  strncpy_P(lcd_status_message, message, 3 * (LCD_WIDTH));
