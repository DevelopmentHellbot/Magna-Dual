...
void Planner::_buffer_steps(const int32_t (&target)[XYZE], float fr_mm_s, const uint8_t extruder) {
  ...
  // For a mixing extruder, get a magnified step_event_count for each [liu]
    ...
    {
    	//if((mixer.rate[i] > 0) && (mixer.rate[i] < 9)) //Set default to 9% if set between 8% to 1% using LCD (Workout to fix Filament blocking)
      		//block->mix_event_count[i] = 11.11 * block->step_event_count;
    	//else if((mixer.rate[i] > 91) && (mixer.rate[i] < 100)) //Set default to 91% if set between 92% to 99% using LCD (Workout to fix Filament blocking)
    		//block->mix_event_count[i] = 1.10 * block->step_event_count;
		//else 
			block->mix_event_count[i] = mixing_factor[i] * block->step_event_count; //All values with LCD is valid
    }
  #endif
